...
public class ConstructBinaryTreeFromInorderAndPostorderTraversal {
    ...
    private TreeNode myBuildTree(int[] inorder, int instart, int inend,
        ...
        TreeNode root = new TreeNode(postorder[postend]);   // postorder end is the root
        int position = findPosition(inorder, instart, inend, postorder[postend]);   // root position

        root.left = myBuildTree(inorder, instart, position - 1,
                postorder, poststart, poststart + position - instart - 1);  // partition in poststart + position - instart
